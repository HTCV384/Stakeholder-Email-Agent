...
- **CRITICAL: Extract FIRST NAME ONLY from {stakeholder_name}**
  * "Kim Schwenk, MSN, RN" → use "kim" (NOT "kim schwenk" or "kim schwenk, msn, rn")
  * "Dr. Sarah Johnson" → use "sarah" (NOT "dr. sarah" or "dr. sarah johnson")
  * "John Smith, MD, MBA" → use "john" (NOT "john smith")
  * "Mary O'Brien, RN" → use "mary" (NOT "mary o'brien")
- Start with "hi [first name]," or "hey [first name]," (lowercase, comma after name)
...
- Examples:
  ...
  * "hi mary, read about your CMS star rating drop to 3 stars"
...
**TONE EXAMPLES:**
✓ "hi sarah, saw memorial's 68% SEP-1 compliance" (first name only!)
✓ "wanna know what's happening with your sepsis patients that doesn't show up in the data?"
...
✗ "hi kim schwenk, msn, rn," (use first name only! should be "hi kim,")
✗ "hi dr. sarah," (no titles! should be "hi sarah,")
...
Generate as JSON:
{
  ...
  "body": "Email body following the structure above. REMEMBER: Use FIRST NAME ONLY in greeting (e.g., 'hi kim,' NOT 'hi kim schwenk, msn, rn,')"
}"""
...
with connection:
    with connection.cursor() as cursor:
        cursor.execute("UPDATE emailTemplates SET promptTemplate = %s WHERE name = 'Casual Insider Approach' AND userId = 0", (updated_prompt,))
        connection.commit()
        print(f"✓ Updated Casual Insider template with strengthened first name extraction ({len(updated_prompt)} chars)")
