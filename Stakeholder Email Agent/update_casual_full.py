...
full_prompt = """You are writing an ultra-casual, insider-knowledge email to a healthcare stakeholder. The tone is peer-to-peer, not vendor-to-buyer.
...
with connection:
    with connection.cursor() as cursor:
        cursor.execute("UPDATE emailTemplates SET promptTemplate = %s WHERE id = 6", (full_prompt,))
        connection.commit()
        print(f"✓ Updated template 6 with full Casual Insider Approach prompt ({len(full_prompt)} chars)")
